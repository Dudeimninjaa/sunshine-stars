...
function getWeeklyStickerInfo(student: Student) {
  const points = Math.max(0, student.total_points || 0);
...
export default function HomePage() {
  ...
  const [competitions, setArchived] = useState<Archived[]>([]);
  const [competitionClassrooms, setArchivedClassrooms] = useState<ArchivedClassroom[]>([]);
  const [competitionScores, setArchivedScores] = useState<ArchivedScore[]>([]);
  ...
  function checkStickerMilestone(student: Student, pointsAdded: number) {
    ...
    const before = Math.max(0, student.total_points || 0);
...
function playSound(type: "positive" | "negative" | "goal" | "captain" | "competition" | "test" = "positive") {
  ...
  async function loadArchived() {
    ...
    setArchived((compRes.data ?? []) as Archived[]);
    setArchivedClassrooms((classRes.data ?? []) as ArchivedClassroom[]);
    setArchivedScores((scoreRes.data ?? []) as ArchivedScore[]);
  }

  async function createArchived() {
    if (!sessionUserId || !newArchivedName.trim()) return;

    const { data, error } = await supabase
      .from("competitions")
      .insert({
        name: newArchivedName.trim(),
        grade_name: newArchivedGrade.trim(),
    ...
    setNewArchivedName("");
    setNewArchivedGrade("");
    setMessage("Archived created.");

    if (data && classroomId) {
      await joinArchived(data.id);
    }
  ...
  async function deleteArchived(competitionId: string, competitionName: string) {
    ...
    if (error) {
      setMessage(`Archived was not deleted: ${error.message}`);
      return;
    }
  ...
  async function joinArchived(competitionId: string) {
    if (!activeClassroom) {
  ...
  async function leaveArchived(competitionId: string) {
    if (!classroomId) return;
  ...
  async function updateArchivedScores(delta: number) {
  ...
  async function resetArchivedScoresForClass() {
    if (!classroomId) return;
    ...
    setMessage("Archived scores for this class were reset to 0.");
    await loadArchived();
  }
  ...
  return (
    <main className={`min-h-screen p-2 md:p-3 bg-gradient-to-br ${theme.bg}`}>
      {centerAnimation && (
      ...
      {!kioskMode && (
        <header className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div><h1 className="text-3xl md:text-4xl font-black text-white drop-shadow-lg">{theme.emoji} Sunshine Stars</h1><p className="text-white/80 text-sm md:text-base">Interactive board mode for teachers</p></div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setMode("board")} className={`px-3 py-2 rounded-xl font-bold flex gap-2 items-center touch-button ${mode === "board" ? `${theme.accent} text-white` : "bg-white"}`}><MonitorPlay /> Board</button>
            <button onClick={() => setMode("setup")} className={`px-3 py-2 rounded-xl font-bold flex gap-2 items-center touch-button ${mode === "setup" ? "bg-blue-500 text-white" : "bg-white"}`}><Settings /> Setup</button>
            <button onClick={() => setMode("reports")} className={`px-3 py-2 rounded-xl font-bold flex gap-2 items-center touch-button ${mode === "reports" ? "bg-emerald-500 text-white" : "bg-white"}`}><BarChart3 /> Reports</button>
      ...
      <section className="grid grid-cols-1 gap-5">
        <div className={`rounded-[2rem] ${theme.panel} shadow-2xl p-3 border border-white`}>
          {!kioskMode && (
            <>
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                <select className="rounded-xl border px-3 py-2 text-base font-bold bg-white" value={classroomId} onChange={(e) => setClassroomId(e.target.value)}>
              ...
              {mode === "setup" && activeClassroom && (
                <div className="mb-3 rounded-xl bg-amber-50 border border-amber-200 p-4">
...
{mode === "setup" && activeClassroom && (
                <details className="mb-3 rounded-xl bg-red-50 border border-red-200 p-4">
        ...
        <aside className={`rounded-[1.5rem] ${theme.panel} shadow-2xl p-3 border border-white`}>
          ...
          {goal ? (
            <>
              ...
              <div className="text-center my-5"><div className="text-5xl animate-pop">{theme.emoji}</div><div className="text-5xl font-black">{goal.current_points} / {goal.target_points}</div><div className="text-slate-500 text-xl">Reward: {goal.reward_name}</div></div>
              <div className="w-full bg-orange-100 rounded-full h-5 overflow-hidden shadow-inner"><div className="bg-gradient-to-r from-yellow-300 to-orange-500 h-full rounded-full transition-all duration-700" style={{ width: `${progress}%` }} /></div>
...
function BoardMode({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-3 items-start xl:h-[calc(100vh-180px)]">
        <section className="board-scroll xl:overflow-y-auto xl:pr-1">
          <div className={`grid grid-cols-2 md:grid-cols-3 ${kioskMode ? "2xl:grid-cols-5" : "2xl:grid-cols-5"} gap-2`}>
            {students.map((student) => (
              <button
                key={student.id}
                onClick={() => setSelectedStudentId(student.id)}
                className={`relative rounded-[1.4rem] p-3 md:p-4 min-h-32 text-center shadow-xl border-4 transition-all touch-button ${
              ...
              >
                <div className="text-4xl md:text-5xl mb-1">{student.avatar}</div>
                ...
                <div className="text-lg md:text-xl font-bold text-orange-500 mt-1">
                  <Star className="inline" /> {student.total_points}
                </div>
                {(() => {
                  const weekly = getWeeklyStickerInfo(student);
                  ...
                  return (
                    <div className="mt-2 rounded-xl bg-slate-100 p-2">
                      <div className="flex items-center justify-between text-xs md:text-sm font-black text-slate-600">
                        <span>Sticker Goal</span>
                        <span>{weekly.progress}/{WEEKLY_STICKER_TARGET}</span>
                      </div>
                      <div className="mt-1 h-2 rounded-full bg-white overflow-hidden">
        ...
        <aside className={`reward-side-panel rounded-[1.5rem] ${theme.soft} p-3 shadow-2xl border border-white xl:sticky xl:top-2 xl:max-h-[calc(100vh-190px)] xl:overflow-y-auto`}>
              ...
              <button
                ...
                className={`rounded-[1.5rem] ${
                  category.points < 0
                    ...
                    : "bg-white"
                } p-3 text-left text-base font-black shadow-lg hover:scale-[1.03] active:scale-95 disabled:opacity-40 transition-all touch-button`}
              >
      ...
      <section className="grid xl:grid-cols-3 gap-2">
        <div className="ui-polish-card rounded-[1.3rem] bg-white p-3 shadow-xl">
        ...
        <div className="ui-polish-card rounded-[1.3rem] bg-white p-3 shadow-xl xl:col-span-2">
